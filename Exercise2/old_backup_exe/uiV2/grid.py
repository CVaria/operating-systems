...
def animate(i):	
	...
	os.system("tail -n 1 mytest > tmpValue")
	#    print fName
	#    os.system("cat /dev/lunix0-temp | sed -e \"/./q\"  > tmpValue ")
	fp=open("tmpValue")
	tmpY = fp.readline()
	fp.close()
	y=float(tmpY)
	...
	#TODO move also start of x axis
	###changing the xmax dynamically
	if i >= xmax:
	    ax.set_xlim(xmin, xmax+(xmax/2))
	    ax.figure.canvas.draw()
	
	#TODO adapt better for all types of measurements
...
def do_plot():
	...
	#Create appropriate Labels and deselect buttons
	if "temp" in fName:
		yLabel="Temperature"
	elif "batt" in fName:
		yLabel="Voltage"	
